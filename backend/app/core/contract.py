"""
Output contract: exact column order and closed allowed-value vocabularies.

Source of truth: ``problem_statement.md`` ("Required output", "Allowed values").
Every value the pipeline emits is coerced to the closest allowed value before it
leaves the system, so a response can never contain an out-of-vocabulary token.

This module is intentionally dependency-free (stdlib only) so it can be imported
from anywhere — pipeline, service, tests — without pulling in heavy SDKs.
"""
...
# ---------------------------------------------------------------------------
def coerce_claim_object(value: str | None) -> str:
    v = (value or "").strip().lower()
    return v if v in CLAIM_OBJECTS else "unknown"
...
def coerce_risk_flags(values: list[str] | None) -> list[str]:
    """Filter to allowed flags, drop 'none', and dedupe (order-preserving)."""
    if not values:
        return []
    seen: list[str] = []
    for raw in values:
...
def risk_flags_to_str(values: list[str] | None) -> str:
    """Join allowed flags with ';'; emit 'none' when empty (CSV convention)."""
    flags = coerce_risk_flags(values)
    return ";".join(flags) if flags else "none"


def as_bool_str(value: bool) -> str:
    """CSV output uses lowercase 'true'/'false' (see sample_claims.csv)."""
    return "true" if value else "false"


@dataclass
class ClaimRecord:
    """One input row from claims.csv / sample_claims.csv / an API request."""
...
class PredictionRow:
    ...
    def to_csv_dict(self) -> dict[str, str]:
        ...
        return {
            ...
            "risk_flags": risk_flags_to_str(self.risk_flags),
    ...
    def to_api_dict(self) -> dict:
        """Coerce to schema-valid values with native JSON types for the API.

        Same coercion as ``to_csv_dict`` but keeps booleans/lists native and
        splits the semicolon-joined ID strings into lists for the JSON client.
        """
        supporting = self.supporting_image_ids or "none"
        supporting_ids = (
            [i for i in supporting.split(";") if i.strip()]
            if supporting and supporting != "none"
            else []
        )
        return {
            "user_id": self.user_id,
            "image_paths": [p for p in self.image_paths.split(";") if p.strip()],
            "user_claim": self.user_claim,
            "claim_object": self.claim_object,
            "evidence_standard_met": bool(self.evidence_standard_met),
            "evidence_standard_met_reason": self.evidence_standard_met_reason.strip(),
            ...
            "valid_image": bool(self.valid_image),
            "severity": coerce_severity(self.severity),
        }
