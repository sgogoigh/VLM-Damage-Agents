...
def decide(
    ...
    # Classify each (claimed part, image) verdict:
    #  - confirming: claimed part visible AND real damage on THAT part
    #  - wrong_part: claimed part visible AND real damage on a DIFFERENT part
    #  - undamaged:  claimed part visible AND no damage
    ...
    for part in claimed_parts:
        for f in matched:
            ...
            if v.actual_issue == "none":
                ...
                # real damage on the claimed part — or the claimed part could not
                # be mapped to vocab, so defer to where the VLM sees the damage.
                confirming.append((part, f, v))
            else:
                wrong_part.append((part, f, v))        # real damage on a different part

    # Welfare check: if the customer explicitly claimed HIGH severity but a
    # usable image judged the visible damage exaggerated, the claim is overstated.
